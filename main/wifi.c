...
static esp_err_t esp_event_handler(void *ctx, system_event_t *event)
{
    ...
    case SYSTEM_EVENT_STA_GOT_IP:
        ...
        ESP_LOGI(TAG, "Connected to WI-FI, IP address: %s",
...
void initialise_wifi(const char *running_partition_label)
{
    ...
    if (wifi_config.sta.ssid[0] == '\0' || wifi_config.sta.password[0] == '\0')
    {
        ...
        {
            ESP_LOGW(TAG,
                            "Factory partition is running, Wi-Fi credentials from config are used and will be saved to the flash memory");
        ...
        {
            ESP_LOGE(TAG,
                            "Wi-Fi credentials were not found, running partition is not '%s'",
                            FACTORY_PARTITION_LABEL);
            APP_ABORT_ON_ERROR(ESP_FAIL);
        }
    } else
    {
        ESP_LOGI(TAG, "Wi-Fi credentials from flash memory: %s, %s",
                        wifi_config.sta.ssid, wifi_config.sta.password);
    }

    APP_ABORT_ON_ERROR(esp_wifi_get_mac(ESP_IF_WIFI_STA, esp32_mac))
    ESP_LOGI(TAG, "MAC address: %02X:%02X:%02X:%02X:%02X:%02X", esp32_mac[0],
                    esp32_mac[1], esp32_mac[2], esp32_mac[3], esp32_mac[4],
                    esp32_mac[5]);
